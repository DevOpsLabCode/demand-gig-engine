...
variable "allow_plan_pull_requests" {
  type        = bool
  description = "Allow pull_request subjects to assume the plan role. Keep false for remote-state plans; the supplied workflow plans only after merge from the protected dev environment."
  default     = false
}
