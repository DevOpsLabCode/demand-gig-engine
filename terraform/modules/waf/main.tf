...
data "aws_iam_policy_document" "logging_kms" {
  ...
  statement {
    ...
    condition {
      ...
      values   = ["arn:${data.aws_partition.current.partition}:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:aws-waf-logs-*"]
    }
  }
}
