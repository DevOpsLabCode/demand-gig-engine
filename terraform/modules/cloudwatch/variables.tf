...
variable "queue_name" {
  type        = string
  description = "Application SQS source queue name."
}

variable "dlq_name" {
  type        = string
  description = "Application SQS dead-letter queue name."
}
...
variable "thresholds" {
  ...
  type = object({
    ...
    queue_visible_messages    = optional(number, 100)
    queue_oldest_message_secs = optional(number, 300)
    rds_cpu_percent           = optional(number, 80)
    rds_free_storage_bytes    = optional(number, 5368709120)
    redis_cpu_percent         = optional(number, 75)
    redis_memory_percent      = optional(number, 80)
    cloudfront_5xx_percent    = optional(number, 5)
  })
  default = {}
  ...
  validation {
    condition = (
      ...
      var.thresholds.queue_visible_messages >= 0 &&
      var.thresholds.queue_oldest_message_secs >= 0 &&
      var.thresholds.rds_cpu_percent > 0 && var.thresholds.rds_cpu_percent <= 100 &&
      var.thresholds.rds_free_storage_bytes > 0 &&
      var.thresholds.redis_cpu_percent > 0 && var.thresholds.redis_cpu_percent <= 100 &&
      var.thresholds.redis_memory_percent > 0 && var.thresholds.redis_memory_percent <= 100 &&
      var.thresholds.cloudfront_5xx_percent >= 0 && var.thresholds.cloudfront_5xx_percent <= 100
    )
