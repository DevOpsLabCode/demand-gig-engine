...
func TestTerraformControlPlaneTrustAndLeastPrivilege(t *testing.T) {
	...
	if !strings.Contains(workflow, `if: github.event_name == 'push' && github.ref == 'refs/heads/main'`) {
		t.Error("Terraform workflow must restrict the AWS-backed development plan to trusted pushes on main")
	}
	...
	if oidcAuthentication == staticAuthentication {
		if oidcAuthentication {
			t.Error("Terraform workflow mixes OIDC role assumption with static AWS access keys; configure exactly one authentication model")
		} else {
			t.Error("Terraform workflow must configure either complete OIDC plan/apply roles or complete static development/production AWS credentials")
		}
	}
	...
	if staticAuthentication {
		if strings.Contains(executableWorkflow, "role-to-assume:") {
			t.Error("static AWS authentication must not retain role-to-assume")
		}
		if strings.Contains(executableWorkflow, "id-token: write") {
			t.Error("static AWS authentication must not request id-token: write")
		}
	}
