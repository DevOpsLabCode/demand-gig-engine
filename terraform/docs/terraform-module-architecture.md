...
| CDN / static assets | `cloudfront`, `s3_static` | OAC, private bucket, KMS, versioning |
| Container registry | `ecr` | immutable tags, KMS, scan-on-push |
| Compute | `ecs_cluster`, `ecs_service` | Fargate, circuit breaker, autoscaling, exec auditing |
| PostgreSQL | `rds_postgres` | encryption, Multi-AZ in prod, Proxy, secret rotation-ready |
| Redis | `redis` | TLS, KMS, replicas/Multi-AZ in prod |
| Async jobs | `messaging` | encrypted SQS, DLQ, EventBridge schedule |
| Email | `ses` | domain identity and DKIM |
| Secrets | `kms`, Secrets Manager | no secrets in state variables or tfvars |
...
6. Apply the complete stack without prompts.
7. Print URLs, role ARNs, and deployment outputs.
