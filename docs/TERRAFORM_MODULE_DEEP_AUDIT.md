...
| `redis` | Ready | CMK encryption, TLS, generated auth token, encrypted runtime URL, private subnets, snapshots, maintenance windows, mandatory replicas and Multi-AZ automatic failover in every environment, and CMK-encrypted one-year engine/slow-query log delivery. | Global datastore/cross-region failover is a DR-layer decision. |
